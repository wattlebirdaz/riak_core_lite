...
-export([sbf/1, sbf/2, sbf/3, sbf/4, bloom/1, bloom/2,
	 member/2, add/2, size/1, capacity/1]).

-export([is_element/2, add_element/2]).
...
bloom(N, E)
    when is_number(N), N > 0, is_float(E), E > 0, E < 1,
...
sbf(N, E, S, R)
    when is_number(N), N > 0, is_float(E), E > 0, E < 1,
	 is_integer(S), S > 0, S < 4, is_float(R), R > 0, R < 1,
	 N >=
	   4 /
	     (E * (1 - R)) -> % rule of thumb; due to double hashing
    #sbf{e = E, s = S, r = R, size = 0,
	 b = [bloom(N, E * (1 - R))]}.
...
member(Elem, #bloom{mb = Mb} = B) ->
    Hashes = make_hashes(Mb, Elem), hash_member(Hashes, B);
...
add(Elem, #bloom{mb = Mb} = B) ->
    Hashes = make_hashes(Mb, Elem), hash_add(Hashes, B);
add(Elem,
    #sbf{size = Size, r = R, s = S, b = [H | T] = Bs} =
	Sbf) ->
    ...
    case hash_member(Hashes, Sbf) of
	  ...
	  case HSize < N of
	    true ->
		Sbf#sbf{size = Size + 1, b = [hash_add(Hashes, H) | T]};
	    false ->
...
hash_add(Hashes,
    ...
    case all_set(Mask, I1, I0, A) of
      ...
      false ->
	  B#bloom{size = Size + 1,
		  a = set_bits(Mask, I1, I0, A, [])}
    end.

set_bits(_Mask, _I1, _I, [], Acc) -> lists:reverse(Acc);
set_bits(Mask, I1, I, [H | T], Acc) ->
    set_bits(Mask, I1, (I + I1) band Mask, T,
	     [bitarray_set(I, H) | Acc]).

bitarray_new(N) ->
    array:new((N - 1) div (?W) + 1, {default, 0}).
...
fixed_case_test_() ->
    {timeout, 100,
     fun () -> fixed_case(bloom(5000), 5000, 0.001) end}.

fixed_case(Bloom, Size, FalseRate) ->
    ?assert((bloom:capacity(Bloom) > Size)),
    ?assertEqual(0, (bloom:size(Bloom))),
    RandomList = simple_shuffle(lists:seq(1, 100 * Size),
				Size),
    [?assertEqual(false, (bloom:is_element(E, Bloom)))
     || E <- RandomList],
    Bloom2 = lists:foldl(fun (E, Bloom0) ->
				 bloom:add_element(E, Bloom0)
			 end,
			 Bloom, RandomList),
    [?assertEqual(true, (bloom:is_element(E, Bloom2)))
     || E <- RandomList],
...
bloom_test() ->
    scalable_case(sbf(1000, 0.2), 1000, 0.2), ok.
