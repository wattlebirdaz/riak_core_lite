        ...
        true ->
            case validate_ssl_config(SSLOpts) of
                {ok, Options} ->
                    Options;
                {error, Reason} ->
                    logger:error("Error, invalid SSL configuration: ~s", [Reason]),
                    false
            end
    end.
...
load_certs([], Acc) ->
    logger:debug("Successfully loaded ~p CA certificates", [length(Acc)]),
    Acc;
load_certs([Cert|Certs], Acc) ->
    case filelib:is_dir(Cert) of
        ...
        _ ->
            logger:debug("Loading certificate ~p", [Cert]),
...
verify_ssl(_, valid_peer, undefined) ->
    logger:error("Unable to determine local certificate's common name"),
...
verify_ssl(Cert, valid_peer, {App, MyCommonName}) ->
    ...
    case invalid_cn_pair(CommonName, MyCommonName) of
        true ->
            logger:error("Peer certificate's common name matches local "
