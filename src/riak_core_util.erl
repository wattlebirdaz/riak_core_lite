...
chash_bucketonly_keyfun({Bucket, _Key}) ->
    chash:key_of(Bucket).
...
-spec
     compose([fun((any()) -> any())]) -> fun((any()) -> any()).
...
get_arch() ->
    string:to_lower(erlang:system_info(system_architecture)).
...
is_arch(Arch) ->
    throw({unsupported_architecture, Arch}).
