...
disable_throttle(AppName, Key) ->
    logger:info("Disabling throttle for ~p/~p.", [AppName, Key]),
...
enable_throttle(AppName, Key) ->
    logger:info("Enabling throttle for ~p/~p.", [AppName, Key]),
...
validate_limits(AppName, Key, Limits) ->
    ...
    case Errors of
        [] ->
            ok;
        Messages ->
            logger:error("Invalid throttle limits for application ~p, activity ~p: ~p.",
...
maybe_log_throttle_change(AppName, Key, NewValue, Reason) ->
    ...
    case NewValue == OldValue of
        true ->
            ok;
        false ->
            logger:info("Changing throttle for ~p/~p from ~p to ~p based on ~ts",
