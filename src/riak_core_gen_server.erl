...
cast({global, Name}, Request) ->
    catch global:send(Name, cast_msg(Request)), ok;
cast({Name, Node} = Dest, Request)
...
do_cast(Dest, Request) ->
    do_send(Dest, cast_msg(Request)), ok.
...
pcast({global, Name}, Priority, Request) ->
    ...
    ok;
pcast({Name, Node} = Dest, Priority, Request)
    when is_atom(Name), is_atom(Node) ->
...
do_cast(Dest, Priority, Request) ->
    do_send(Dest, cast_msg(Priority, Request)), ok.

cast_msg(Priority, Request) ->
...
do_abcast([Node | Nodes], Name, Msg)
    when is_atom(Node) ->
    do_send({Name, Node}, Msg), do_abcast(Nodes, Name, Msg);
...
unregister_name(Pid) when is_pid(Pid) -> Pid.
...
%%% ---------------------------------------------------
do_send(Dest, Msg) -> catch erlang:send(Dest, Msg).
...
send_nodes([], _Name, _Tag, _Req, Monitors) -> Monitors.
...
unmonitor(Ref) when is_reference(Ref) ->
    ...
    receive
      {'DOWN', Ref, _, _, _} -> true after 0 -> true
    end.
...
opt(Op, [{Op, Value} | _]) -> {ok, Value};
opt(Op, [_ | Options]) -> opt(Op, Options);
opt(_, []) -> false.
...
get_proc_name({global, Name}) ->
    case global:whereis_name(Name) of
      ...
      Pid when Pid =:= self() -> Name;
      _Pid -> exit(process_not_registered_globally)
    end.
