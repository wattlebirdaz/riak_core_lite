...
%% API
-export([start_link/0, service_up/2, service_up/3,
	 service_up/4, check_health/1, suspend_health_checks/0,
	 resume_health_checks/0, service_down/1, service_down/2,
...
-record(state,
	{status = up, services = [], health_checks = [],
	 healths_enabled = true, peers = [], avsn = 0,
	 bcast_tref, bcast_mod = {gen_server, abcast}}).
...
-record(health_check,
	{state = waiting  :: waiting | checking | suspend,
	 ...
	 check_interval = ?DEFAULT_HEALTH_CHECK_INTERVAL  ::
	     timeout(),
	 max_callback_failures = 3, max_health_failures = 1}).
...
-type health_opt() :: hc_check_interval_opt() |
		      hc_max_callback_fails_opt() | hc_max_health_fails_opt().
...
check_health(Service) ->
    (?MODULE) ! {check_health, Service}, ok.
...
service_down(Id, true) ->
    gen_server:call(?MODULE,
		    {service_down, Id, health_check}, infinitiy);
...
services(Node) ->
    case check_node_valid(Node) of
      true -> internal_get_services(Node);
      _ -> invalid_node
    end.

nodes(Service) -> internal_get_nodes(Service).
...
avsn() -> gen_server:call(?MODULE, get_avsn, infinity).

set_broadcast_module(Module, Fn) ->
    gen_server:call(?MODULE, {set_bcast_mod, Module, Fn},
		    infinity).
...
handle_call({service_up, Id, Pid, MFA, Options}, From,
	...
	of
	  ...
	  CheckInterval = proplists:get_value(check_interval,
					      Options,
	  ...
	  CheckRec = #health_check{callback = MFA,
				   check_interval = CheckInterval,
				   service_pid = Pid,
				   max_health_failures =
				       proplists:get_value(max_health_failures,
							   Options, 1),
				   max_callback_failures =
				       proplists:get_value(max_callback_failures,
							   Options, 3),
				   interval_tref = IntervalTref},
	  Healths = orddict:store(Id, CheckRec,
				  State2#state.health_checks);
      false -> Healths = State2#state.health_checks
    end,
...
handle_call({node_status, Status}, _From, State) ->
    ...
    S2 = case Transition of
	   {up, down} -> %% up -> down
		 ...
		 false -> Healths = State#state.health_checks
	       end,
...
handle_cast({down, Node}, State) ->
    node_down(Node, State), {noreply, update_avsn(State)};
handle_cast({health_check_result, Pid, R}, State) ->
    Service = erlang:erase(Pid),
    State2 = handle_check_msg({result, Pid, R}, Service,
			      State),
    {noreply, State2}.
...
handle_info({'DOWN', Mref, _, _Pid, _Info}, State) ->
    ...
    case erlang:get(Mref) of
	  ...
	  Services = ordsets:del_element(Id,
...
handle_info(broadcast, State) ->
    S2 = broadcast(State#state.peers, State), {noreply, S2}.
...
schedule_broadcast(State) ->
    case State#state.bcast_tref of
      undefined -> ok;
      OldTref -> _ = erlang:cancel_timer(OldTref), ok
    end,
...
is_node_up(Node) -> ets:member(?MODULE, Node).
...
handle_check_msg(Msg, ServiceId, State) ->
    case orddict:find(ServiceId, State#state.health_checks)
	of
      error -> State;
      {ok, Check} ->
...
handle_check_return({ok, Check}, ServiceId, State) ->
    Healths = orddict:store(ServiceId, Check,
			    State#state.health_checks),
    State#state{health_checks = Healths};
...
health_fsm(Msg, Service,
	   #health_check{state = StateName} = Check) ->
    {Reply, NextState, Check2} = health_fsm(StateName, Msg,
					    Service, Check),
...
health_fsm(checking, {result, Pid, Cause}, Service,
	...
	InCheck,
    {Reply, HPFails1} = handle_fsm_exit(Cause, HPFails,
					HPMaxFails),
    Tref = next_health_tref(HPFails1,
			    InCheck#health_check.check_interval, Service),
...
health_fsm(waiting, suspend, _Service, InCheck) ->
    case InCheck#health_check.interval_tref of
      ...
      _ ->
	  _ =
	      erlang:cancel_timer(InCheck#health_check.interval_tref),
	  ok
    end,
    {ok, suspend,
     InCheck#health_check{interval_tref = undefined}};
...
handle_fsm_exit(true, HPFails, MaxHPFails)
    when HPFails >= MaxHPFails ->
...
handle_fsm_exit(true, HPFails, MaxHPFails)
    ...
    {ok, 0};
handle_fsm_exit(false, HPFails, MaxHPFails)
    when HPFails + 1 == MaxHPFails ->
...
start_health_check(Service,
    ...
    CheckingPid = proc_lib:spawn_link(fun () ->
					      case erlang:apply(Mod, Func,
								[Pid | Args])
						  of
						R
						    when R =:= true orelse
							   R =:= false ->
						    health_check_result(self(),
									R);
						Else -> exit(Else)
					      end
				      end),
...
all_health_fsms(Msg, Healths) ->
    [begin {ok, C1} = health_fsm(Msg, S, C), {S, C1} end
     || {S, C} <- Healths].

determine_time(Failures, BaseInterval)
    when Failures < 4 ->
    BaseInterval;
determine_time(Failures, BaseInterval)
    when Failures < 11 ->
