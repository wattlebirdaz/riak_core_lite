...
r_run(Input, Command) ->
    case r_port() of
	  ...
	  port_command(Port,
		       ?FMT("write(~s, ncolumns=1, file=stdout())\n",
			    [Command])),
...
r_port() ->
    ...
    try port_command(Port, "write('', file=stdout())\n") of
      _ ->
	  receive
	    {Port, {data, {eol, []}}} -> {ok, Port};
	    {Port, {data, {eol, Other}}} ->
		erlang:erase(r_port), {error, Other}
	  end
    catch
      error:badarg ->
	  erlang:erase(r_port), {error, port_closed}
    end.

r_simple_read_loop(Port, Acc) ->
    receive
      {Port, {data, {eol, []}}} -> lists:reverse(Acc);
      {Port, {data, {eol, Line}}} ->
	  case Line of
	    "Error" ++ _ ->
		Error = get_error(Port, [Line]), exit({error, Error});
	    _ -> r_simple_read_loop(Port, [to_number(Line) | Acc])
	  end;
      {Port, {exit_status, _}} -> lists:reverse(Acc)
    end.
