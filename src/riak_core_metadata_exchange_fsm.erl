...
prepare(timeout, State=#state{peer=Peer}) ->
    ...
    logger:error("metadata exchange with ~p timed out aquiring locks", [Peer]),
    {stop, normal, State};
...
update(timeout, State=#state{peer=Peer}) ->
    logger:error("metadata exchange with ~p timed out updating trees", [Peer]),
    {stop, normal, State};
...
exchange(timeout, State=#state{peer=Peer}) ->
    ...
    case Total > 0 of
        true ->
            logger:info("completed metadata exchange with ~p. repaired ~p missing local prefixes, "
                       "~p missing remote prefixes, and ~p keys", [Peer, LocalPrefixes, RemotePrefixes, Keys]);
        false ->
            logger:debug("completed metadata exchange with ~p. nothing repaired", [Peer])
    end,
    {stop, normal, State}.
