...
-export([fresh/0, fresh/2, descends/2, dominates/2,
	 descends_dot/2, pure_dot/1, merge/1, get_counter/2,
	 get_timestamp/2, get_dot/2, valid_dot/1, increment/2,
	 increment/3, all_nodes/1, equal/2, prune/3,
	 timestamp/0]).
...
merge([AClock | VClocks], NClock) ->
    merge(VClocks,
	  merge(lists:keysort(1, AClock), NClock, [])).
...
merge(V = [{Node1, {Ctr1, TS1} = CT1} = NCT1 | VClock],
      N = [{Node2, {Ctr2, TS2} = CT2} = NCT2 | NClock],
      AccClock) ->
    if Node1 < Node2 -> merge(VClock, N, [NCT1 | AccClock]);
       Node1 > Node2 -> merge(V, NClock, [NCT2 | AccClock]);
       true ->
...
-spec get_counter(Node :: vclock_node(),
...
-spec get_timestamp(Node :: vclock_node(),
		    VClock :: vclock()) -> timestamp() | undefined.
...
-spec get_dot(Node :: vclock_node(),
	      VClock :: vclock()) -> {ok, dot()} | undefined.
...
valid_dot({_, {Cnt, TS}})
    when is_integer(Cnt), is_integer(TS) ->
    true;
valid_dot(_) -> false.
...
% @doc Increment VClock at Node.
-spec increment(Node :: vclock_node(),
		IncTs :: timestamp(), VClock :: vclock()) -> vclock().

increment(Node, IncTs, VClock) ->
    {{_Ctr, _TS} = C1, NewV} = case lists:keytake(Node, 1,
						  VClock)
				   of
				 false -> {{1, IncTs}, VClock};
				 {value, {_N, {C, _T}}, ModV} ->
				     {{C + 1, IncTs}, ModV}
			       end,
    [{Node, C1} | NewV].
...
-define(SECONDS_FROM_GREGORIAN_BASE_TO_EPOCH,
	(?DAYS_FROM_GREGORIAN_BASE_TO_EPOCH) * 24 * 60 *
	  60).    %% == calendar:datetime_to_gregorian_seconds({{1970,1,1},{0,0,0}})
...
-spec equal(VClockA :: vclock(),
	    VClockB :: vclock()) -> boolean().

equal(VA, VB) -> lists:sort(VA) =:= lists:sort(VB).

% @doc Possibly shrink the size of a vclock, depending on current age and size.
-spec prune(V :: vclock(), Now :: integer(),
	    BucketProps :: term()) -> vclock().
...
prune(V, Now, BucketProps) ->
    ...
    SortV = lists:sort(fun ({N1, {_, T1}}, {N2, {_, T2}}) ->
			       {T1, N1} < {T2, N2}
		       end,
		       V),
...
prune_vclock1(V, Now, BProps) ->
    case length(V) =< get_property(small_vclock, BProps) of
      ...
      false ->
	  {_, {_, HeadTime}} = hd(V),
	  case Now - HeadTime < get_property(young_vclock, BProps)
	      of
	    true -> V;
...
prune_small_test() ->
    ...
    SmallVC = [{<<"1">>, {1, OldTime}},
	       {<<"2">>, {2, OldTime}}, {<<"3">>, {3, OldTime}}],
    Props = [{small_vclock, 4}],
    ?assertEqual((lists:sort(SmallVC)),
		 (lists:sort(prune(SmallVC, Now, Props)))).

prune_young_test() ->
    % vclock with all entries younger than young_vclock will be untouched
    Now = riak_core_util:moment(),
    NewTime = Now - 1,
    VC = [{<<"1">>, {1, NewTime}}, {<<"2">>, {2, NewTime}},
	  {<<"3">>, {3, NewTime}}],
    Props = [{small_vclock, 1}, {young_vclock, 1000}],
    ?assertEqual((lists:sort(VC)),
		 (lists:sort(prune(VC, Now, Props)))).
...
prune_order_test() ->
    ...
    Props = [{small_vclock, 1}, {young_vclock, 1},
	     {big_vclock, 2}, {old_vclock, 10000}],
    ?assertEqual((prune(VC1, Now, Props)),
		 (prune(VC2, Now, Props))).
...
get_entry_test() ->
    VC = vclock:fresh(),
    VC1 = increment(a,
		    increment(c, increment(b, increment(a, VC)))),
...
valid_entry_test() ->
    ...
    [begin
       {ok, E} = get_dot(Actor, VC1), ?assert((valid_dot(E)))
     end
     || Actor <- [a, b, c]],
